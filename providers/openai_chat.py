...
class OpenAIChatHandler(ProviderHandler, OpenAIFakeNonStream, OpenAIFCEnhance):
    ...
    async def _handle_stream_fc_hook(
        ...
        target_tc = self._find_failed_function_call(assembled, tools)
        if target_tc is None and self._hint_tools:
            target_tc = self._find_hinted_empty_tc(assembled)
        is_failed = target_tc is not None
        ...
        fn_name = (target_tc.get("function") or {}).get("name", "")
        extracted = await self._extract_args_as_json(clean_body, fn_name, sub_path, headers,
                                                     model_reply=model_reply)
        if extracted is not None:
            _before = (target_tc.get("function") or {}).get("arguments", "{}")
        ...
        if is_failed:
            ...
            for attempt in range(self.fix_retries):
                ...
                failed_tc = self._find_failed_function_call(result, tools)
                ...
                fn2 = (failed_tc.get("function") or {}).get("name", "")
                ...
                if extracted is not None:
                    ...
                    self._log_fc_modify("openai", 1, fn2, _before, extracted,
    ...
    async def handle(self, req: web.Request, sub_path: str) -> web.Response:
        ...
        if client_wants_stream:
            if (not clean_body.get("tools") and not clean_body.get("functions")) or self.fc_enhance == 0:
        ...
        target_tc = self._find_failed_function_call(result, clean_body.get("tools", []))
        if target_tc is None and self._hint_tools:
            target_tc = self._find_hinted_empty_tc(result)
        is_failed = target_tc is not None
        ...
        function_name = (target_tc.get("function") or {}).get("name", "")
        # 提取模型回复文本
        ...
        if is_failed:
            for attempt in range(self.fix_retries):
                ...
                extracted = await self._extract_args_as_json(
                    clean_body, fn2, sub_path, headers,
